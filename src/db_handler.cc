...
DbHandler::DbHandler(const string& db_uri)
    : db_inst_{make_unique<instance>()}, db_conn_{uri{db_uri}} {}

bool DbHandler::FindPlaylist(const string& name) const {
  auto playlists = db_conn_["espotifai-db"]["playlists"];
  auto found = playlists.find_one(document{} << "name" << name << finalize);

  return found ? true : false;
}

bool DbHandler::FindMusicInPlaylist(const string& uri,
                                    const string& playlist) const {
  // TODO: improve this method.
  auto playlists = db_conn_["espotifai-db"]["playlists"];
  auto found = playlists.find_one(document{} << "name" << playlist << finalize);
...
void DbHandler::CreatePlaylist(const string& name) const {
  ...
  value doc = builder << "name" << name << "owner"
                      << ""
                      << "musics" << open_array << close_array << finalize;
...
void DbHandler::AddMusic(const MusicInfo& music,
  ...
  playlists.update_one(
      ...
      make_document(
          kvp("$addToSet", document{} << "musics" << open_document << "name"
                                      << music.name << "artist" << music.artist
                                      << "uri" << music.uri << "duration"
                                      << to_string(music.duration)
...

  {
    ...
    for (auto& music : music_list) {
      ...
      MusicInfo info = {
          ...
          .duration = std::atoi(music_doc["duration"].get_utf8().value.data())};
...
vector<string> DbHandler::GetPlaylists() const {
  ...
  for (auto& playlist : playlists.find({})) {
    list_playlists.emplace_back(playlist["name"].get_utf8().value.data());
  }
